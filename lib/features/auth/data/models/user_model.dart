...
class UserModel {
  ...
  // Legacy per-court assignments retained for existing accounts.
  ...
  /// True if this account can reach an admin surface.
  bool get hasAdminAccess =>
      isAdmin || managedVenueIds.isNotEmpty || managedCourtIds.isNotEmpty;
  ...
  factory UserModel.fromJson(Map<String, dynamic> json) {
    return UserModel(
      ...
      preferredSide:
          json['preferredSide'] as String? ?? AppConstants.sideForerhand,
      matchesPlayed: json['matchesPlayed'] as int? ?? 0,
      phone: json['phone'] as String?,
      managedVenueIds: List<String>.from(
        json['managedVenueIds'] as List? ?? [],
      ),
      managedCourtIds: List<String>.from(
        json['managedCourtIds'] as List? ?? [],
      ),
      createdAt: (json['createdAt'] as Timestamp?)?.toDate() ?? DateTime.now(),
      favoriteVenueIds: List<String>.from(
        json['favoriteVenueIds'] as List? ?? [],
      ),
      walletBalance: (json['walletBalance'] as num?)?.toDouble() ?? 0.0,
      loyaltyBookingCount: json['loyaltyBookingCount'] as int? ?? 0,
      loyaltyDiscountEligible:
          json['loyaltyDiscountEligible'] as bool? ?? false,
      fcmToken: json['fcmToken'] as String?,
      phoneVerified: json['phoneVerified'] as bool? ?? false,
      phoneVerificationSkipped:
          json['phoneVerificationSkipped'] as bool? ?? false,
    );
  }
  ...
  UserModel copyWith({
    ...
    return UserModel(
      ...
      loyaltyDiscountEligible:
          loyaltyDiscountEligible ?? this.loyaltyDiscountEligible,
      fcmToken: fcmToken ?? this.fcmToken,
      phoneVerified: phoneVerified ?? this.phoneVerified,
      phoneVerificationSkipped:
          phoneVerificationSkipped ?? this.phoneVerificationSkipped,
    );
  }
}
