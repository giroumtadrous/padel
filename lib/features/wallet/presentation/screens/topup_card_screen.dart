...
/// Card details entry for wallet top-up. Reached from [WalletScreen] once the
/// user has entered an amount — mirrors CardPaymentScreen's card UI/formatters
/// so the two payment flows feel identical.
class TopUpCardScreen extends StatefulWidget {
  final double amount;

  const TopUpCardScreen({super.key, required this.amount});
...
class _TopUpCardScreenState extends State<TopUpCardScreen> {
  ...
  Future<void> _submit() async {
    ...
    context.read<AuthBloc>().add(TopUpWallet(widget.amount));

    if (!mounted) return;
    Navigator.pop(context);
    ScaffoldMessenger.of(context).showSnackBar(
      SnackBar(content: Text('EGP ${widget.amount.toInt()} added to your wallet!')),
    );
  }
        ...
        child: SingleChildScrollView(
          ...
          child: Form(
            ...
            child: Column(
              ...
              children: [
                Text(
                  'Adding EGP ${widget.amount.toInt()} to your wallet',
                ...
                TextFormField(
                  ...
                  decoration: InputDecoration(
                    ...
                    suffixIcon: _buildNetworkBadge(),
                  ),
                  validator: (v) {
                    final digits = (v ?? '').replaceAll(' ', '');
                    if (digits.length < 16) return 'Enter a valid 16-digit card number';
                    if (detectCardNetwork(digits) == CardNetwork.unknown) {
  ...
  Widget? _buildNetworkBadge() {
    ...
    final network = detectCardNetwork(digits);
    if (network == CardNetwork.unknown) return null;
    return Padding(
      padding: const EdgeInsets.symmetric(horizontal: 12),
      child: Text(
        network == CardNetwork.visa ? 'VISA' : 'Mastercard',
